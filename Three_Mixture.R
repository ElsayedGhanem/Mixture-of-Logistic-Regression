...
## first population
Beta01 <-  c(2.85,-10,-5.11)
Beta02 <-  c(10,9.90,5.11)
Beta03 <-  c(-3.84,9.90,5.11)

Beta0 <- rbind(Beta01,Beta02,Beta03)
p0 <- ncol(Beta0) 
maxit_em0 <- 200
epsilon_em0 <-1e-5 
tol_irwls0 <- 1e-3
...
Design_Matrix <- function(n,p,phi)
{
...
irwls_Ridge_one_iteration <- function(X, y,La ,beta.k, beta.old)
{
  ...
  if(is.nan(eigen(V)$values)) return(b_old2)
  eV <- eigen(V)$values
...
irwls_Liu_one_iteration <- function(X, y, La ,dFix ,beta.k, beta.old)
{
  pp <- ncol(X)
  IPP <- diag(pp) 
  b_old2 <- beta.old
...
em_ml <-function(epsilon,x,y,beta.0,beta.old,pi.0,pi.old,maxit_EM,tol_irwls,printout)
{
  pp <- dim(x)[2]
  M <- length(pi.0)  
  ...
  for(k in 1:maxit_EM)
  {
    if(any(is.na(beta.k)))
    {
      MSE_pi <- .msepi_one_iteration(pi.old[c(1,2)],pi0[c(1,2)])
      MSE_Beta <- .msebeta_one_iteration(beta.old,Beta0)
      if(k==1) return(list(k=k,pi=c(pi.old,MSE_pi,MSE_Beta,k,1),beta=beta.old))
      result <- c(pi.old,MSE_pi,MSE_Beta,k,101)
      ret <- list(k=k,pi=round(result,3),beta=beta.old)
      return(ret)
    }
    ...
    if(any(is.na(pi.k)))
    {
      ...
      MSE_Beta <- .msebeta_one_iteration(beta.old,Beta0)
      if(k==1) return(list(k=k,pi=c(pi.old,MSE_pi,MSE_Beta,k,1),beta=beta.old))
      result <- c(pi.old,MSE_pi,MSE_Beta,k,102)
      ret <- list(k=k,pi=round(result,3),beta=beta.old)
      return(ret)
    }
    ...
    if(any(pi.k<0.01))
    {
      ...
      MSE_Beta <- .msebeta_one_iteration(beta.old,Beta0)
      if(k==1) return(list(k=k,pi=c(pi.old,MSE_pi,MSE_Beta,k,1),beta=beta.old))
      result <- c(pi.old,MSE_pi,MSE_Beta,k,103)
      ret <- list(k=k,pi=round(result,3),beta=beta.old)
      return(ret)
    }
    ...
    if(any(is.na(tau)))
    {
      ...
      MSE_Beta <- .msebeta_one_iteration(beta.old,Beta0)
      if(k==1) return(list(k=k,pi=c(pi.old,MSE_pi,MSE_Beta,k,1),beta=beta.old))
      result <- c(pi.old,MSE_pi,MSE_Beta,k,104)
      ret <- list(k=k,pi=round(result,3),beta=beta.old)
      return(ret)
    }
    Z <- Zstoc(tau=tau)
    if(any(is.na(Z)))
    {
      ...
      MSE_Beta <- .msebeta_one_iteration(beta.old,Beta0)
      if(k==1) return(list(k=k,pi=c(pi.old,MSE_pi,MSE_Beta,k,1),beta=beta.old))
      result <- c(pi.old,MSE_pi,MSE_Beta,k,105)
      ret <- list(k=k,pi=round(result,3),beta=beta.old)
      return(ret)
    }
    ...
    for (j in 1:ncol(Z)) 
    {
      ...
      if(sum(zInd)==1)
      {
        ...
        MSE_Beta <- .msebeta_one_iteration(beta.old,Beta0)
        if(k==1) return(list(k=k,pi=c(pi.old,MSE_pi,MSE_Beta,k,1),beta=beta.old))
        result <- c(pi.old,MSE_pi,MSE_Beta,k,106)
        ret <- list(k=k,pi=round(result,3),beta=beta.old)
        return(ret)
      }
    ...
    changeValuePi   <- sqrt(sum(pi.new[c(1,2)]-pi.k[c(1,2)]^2))
  ...
  MSE_pi <- .msepi_one_iteration(pi.new[c(1,2)],pi0[c(1,2)])
  MSE_Beta <- .msebeta_one_iteration(beta.new,Beta0)
  result <- c(pi.new,MSE_pi,MSE_Beta,k,100)
  ret <- list(k=k,pi=round(result,3),beta=beta.new)
  return(ret)
}

em_ridge <-function(epsilon,x,y,beta.0,beta.old,pi.0,pi.old,maxit_EM,tol_irwls,Beta_Hat,printout)
{
  pp <- dim(x)[2]
  M <- length(pi.0)  
  ...
  for(k in 1:maxit_EM)
  {
    if(any(is.na(beta.k)))
    {
      MSE_pi <- .msepi_one_iteration(pi.old[c(1,2)],pi0[c(1,2)])
      MSE_Beta <- .msebeta_one_iteration(beta.old,Beta0)
      if(k==1) return(list(k=k,pi=c(pi.old,0,0,0,MSE_pi,MSE_Beta,k,1),beta=beta.old))
      pi_Lamda <- c(pi.old,Lamda,MSE_pi,MSE_Beta,k,101)
    ...
    if(any(is.na(pi.k)))
    {
      ...
      MSE_Beta <- .msebeta_one_iteration(beta.old,Beta0)
      if(k==1) return(list(k=k,pi=c(pi.old,0,0,0,MSE_pi,MSE_Beta,k,1),beta=beta.old))
      pi_Lamda <- c(pi.old,Lamda,MSE_pi,MSE_Beta,k,102)
      ret <- list(k=k,pi=round(pi_Lamda,3),beta=beta.old)
      return(ret)
    }
    ...
    if(any(pi.k<0.01))
    {
      ...
      MSE_Beta <- .msebeta_one_iteration(beta.old,Beta0)
      if(k==1) return(list(k=k,pi=c(pi.old,0,0,0,MSE_pi,MSE_Beta,k,1),beta=beta.old))
      pi_Lamda <- c(pi.old,Lamda,MSE_pi,MSE_Beta,k,103)
      ret <- list(k=k,pi=round(pi_Lamda,3),beta=beta.old)
      return(ret)
    }
    ...
    if(any(is.na(tau)))
    {
      ...
      MSE_Beta <- .msebeta_one_iteration(beta.old,Beta0)
      if(k==1) return(list(k=k,pi=c(pi.old,0,0,0,MSE_pi,MSE_Beta,k,1),beta=beta.old))
      pi_Lamda <- c(pi.old,Lamda,MSE_pi,MSE_Beta,k,104)
      ret <- list(k=k,pi=round(pi_Lamda,3),beta=beta.old)
      return(ret)
    }
    Z <- Zstoc(tau=tau)
    if(any(is.na(Z)))
    {
      ...
      MSE_Beta <- .msebeta_one_iteration(beta.old,Beta0)
      if(k==1) return(list(k=k,pi=c(pi.old,0,0,0,MSE_pi,MSE_Beta,k,1),beta=beta.old))
      pi_Lamda <- c(pi.old,Lamda,MSE_pi,MSE_Beta,k,105)
      ret <- list(k=k,pi=round(pi_Lamda,3),beta=beta.old)
      return(ret)
    }
    ...
    for (j in 1:ncol(Z)) 
    {
      ...
      if(sum(zInd)==1)
      {
        ...
        MSE_Beta <- .msebeta_one_iteration(beta.old,Beta0)
        if(k==1) return(list(k=k,pi=c(pi.old,0,0,0,MSE_pi,MSE_Beta,k,1),beta=beta.old))
        pi_Lamda <- c(pi.old,Lamda,MSE_pi,MSE_Beta,k,106)
        ret <- list(k=k,pi=round(pi_Lamda,3),beta=beta.old)
        return(ret)
      }
  ...
  MSE_pi <- .msepi_one_iteration(pi.new[c(1,2)],pi0[c(1,2)])
  MSE_Beta <- .msebeta_one_iteration(beta.new,Beta0)
  pi_Lamda <- c(pi.new,Lamda,MSE_pi,MSE_Beta,k,100)
...
em_Liu <-function(epsilon,x,y,beta.0,beta.old,pi.0,pi.old,maxit_EM,tol_irwls,Beta_Hat,printout=F)
{
  ...
  for(k in 1:maxit_EM)
  {
    if(any(is.na(beta.k)))
    {
      MSE_pi <- .msepi_one_iteration(pi.old[c(1,2)],pi0[c(1,2)])
      MSE_Beta <- .msebeta_one_iteration(beta.old,Beta0)
      if(k==1) return(list(k=k,pi=c(pi.old,0,0,0,0,0,0,MSE_pi,MSE_Beta,k,1),beta=beta.old))
      pi_Lamda_dopt <- c(pi.old,Lamda,dopt,MSE_pi,MSE_Beta,k,101)
    ...
    if(any(is.na(pi.k)))
    {
      ...
      MSE_Beta <- .msebeta_one_iteration(beta.old,Beta0)
      if(k==1) return(list(k=k,pi=c(pi.old,0,0,0,0,0,0,MSE_pi,MSE_Beta,k,1),beta=beta.old))
      pi_Lamda_dopt <- c(pi.old,Lamda,dopt,MSE_pi,MSE_Beta,k,102)
      ret <- list(k=k,pi=round(pi_Lamda_dopt,3),beta=beta.old)
      return(ret)
    }
    ...
    if(any(pi.k<0.01))
    {
      ...
      MSE_Beta <- .msebeta_one_iteration(beta.old,Beta0)
      if(k==1) return(list(k=k,pi=c(pi.old,0,0,0,0,0,0,MSE_pi,MSE_Beta,k,1),beta=beta.old))
      pi_Lamda_dopt <- c(pi.old,Lamda,dopt,MSE_pi,MSE_Beta,k,103)
      ret <- list(k=k,pi=round(pi_Lamda_dopt,3),beta=beta.old)
      return(ret)
    }
    ...
    if(any(is.na(tau)))
    {
      ...
      MSE_Beta <- .msebeta_one_iteration(beta.old,Beta0)
      if(k==1) return(list(k=k,pi=c(pi.old,0,0,0,0,0,0,MSE_pi,MSE_Beta,k,1),beta=beta.old))
      pi_Lamda_dopt <- c(pi.old,Lamda,dopt,MSE_pi,MSE_Beta,k,104)
      ret <- list(k=k,pi=round(pi_Lamda_dopt,3),beta=beta.old)
      return(ret)
    }
    Z <- Zstoc(tau=tau)
    if(any(is.na(Z)))
    {
      ...
      MSE_Beta <- .msebeta_one_iteration(beta.old,Beta0)
      if(k==1) return(list(k=k,pi=c(pi.old,0,0,0,0,0,0,MSE_pi,MSE_Beta,k,1),beta=beta.old))
      pi_Lamda_dopt <- c(pi.old,Lamda,dopt,MSE_pi,MSE_Beta,k,105)
      ret <- list(k=k,pi=round(pi_Lamda_dopt,3),beta=beta.old)
      return(ret)
    }
    ...
    for (j in 1:ncol(Z)) 
    {
      ...
      if(sum(zInd)==1)
      {
        ...
        MSE_Beta <- .msebeta_one_iteration(beta.old,Beta0)
        if(k==1) return(list(k=k,pi=c(pi.old,0,0,0,0,0,0,MSE_pi,MSE_Beta,k,1),beta=beta.old))
        pi_Lamda_dopt <- c(pi.old,Lamda,dopt,MSE_pi,MSE_Beta,k,106)
        ret <- list(k=k,pi=round(pi_Lamda_dopt,3),beta=beta.old)
        return(ret)
      }
      else
      ...
      beta_Liu <- irwls_Liu_one_iteration(X=x[zInd,],y=y[zInd],La=LamdaJ,dFix=dFixJ,
                                beta.k=beta.k[j,],beta.old=beta.old[j,])
      Beta_Liu[j,] <-  beta_Liu
      Lamda[j] <- LamdaJ
      dopt[j] <- dFixJ 
  ...
  MSE_pi <- .msepi_one_iteration(pi.new[c(1,2)],pi0[c(1,2)])
  MSE_Beta <- .msebeta_one_iteration(beta.new,Beta0)
  pi_Lamda_dopt <- c(pi.new,Lamda,dopt,MSE_pi,MSE_Beta,k,100)
...
{
  dif1 <- sqrt((t(P_hat)-P_true)^2)
  dif2 <- colSums(dif1)
  mean_pi <- round(mean(dif2),3)
  quan_pi <- round(quantile(dif2, probs=c(0.025,0.5,0.975)),3)
...
Simulation <- function(n,p,phi,epsilon,M,Beta,beta.0,beta.old,pi,pi.0,pi.old,maxit_EM,tol_irwls,printout=F)
{
  resArray_LS <-resArray_Ridge <- resArray_Liu <- array(NA,dim=c(M,p,Nsim))
  MO_LS_pi <- matrix(NA, nrow = Nsim, ncol = length(pi)+4)
  
  MO_Ridge_pi <- matrix(NA, nrow = Nsim, ncol = length(pi)+7)
  
  MO_Liu_pi <- matrix(NA, nrow = Nsim, ncol = (length(pi)*4)+1)
  
  for (j in 1:Nsim)
  {
    cat("j = ",j)
    x <- Design_Matrix(n=n,p=p,phi=phi)
    ...
    EM_Result_Ridge <-em_ridge(epsilon=epsilon_em0,x=x,y=y,beta.0=Beta.01,
                            ...
                            tol_irwls=tol_irwls0,Beta_Hat=B_ml,printout)
...
Wrapping <- function(M_hat_LS,M_hat_Ridge,M_hat_Liu,B_true,Pi_hat_LS,Pi_hat_Ridge,Pi_hat_Liu,Pi_true)
{
  ...
  EMSE_LS_Pi     <- .msepi(P_hat =  Pi_hat_LS[,1:2],P_true = Pi_true[1:2])

  EMSE_Ridge_Beta     <- .mseBeta(Mhat =  M_hat_Ridge,Btrue = Barray)
  EMSE_Ridge_Pi     <- .msepi(P_hat =  Pi_hat_Ridge[,1:2],P_true = Pi_true[1:2])

  EMSE_Liu_Beta     <- .mseBeta(Mhat =  M_hat_Liu,Btrue = Barray)
  EMSE_Liu_Pi     <- .msepi(P_hat =  Pi_hat_Liu[,1:2],P_true = Pi_true[1:2])
...
Generate_Testing <- function(n,p,phi,Beta,pi,M)
{
  x <- Design_Matrix(n=n,p=p,phi=phi)
  Yres <- Design_Response_Mixture(x=x,beta=Beta,pi=pi,M=M)
  y <- Yres[,1]
...
Testing <- function(MO_LS,MO_Ridge,MO_Liu,MO_Pi_LS,MO_Pi_Ridge,MO_Pi_Liu,M)
{
  ...
  for (i in 1:Nsim) 
  {
    Test_Data <- Generate_Testing(n=n0test,p=p0,phi=phi0t,Beta=Beta0,pi=pi0,M=M0)
    X_test=Test_Data$x
    Y_test=Test_Data$y
...
Sim_Result=Simulation(n=n0t,p=p0,phi=phi0t,epsilon=epsilon_em0,M=M0,
                      Beta=Beta0,beta.0=Beta.01,beta.old=Beta.02,
                      pi=pi0,pi.0=pi.01,pi.old=pi.02,maxit_EM=maxit_em0,
                      tol_irwls=tol_irwls0,printout = F)

MO_LS <- Sim_Result$MO_LS_Beta
MO_Pi_LS_1 <- Sim_Result$MO_LS_pi
MO_Pi_LS <- MO_Pi_LS_1[,1:3]

MO_Ridge <- Sim_Result$MO_Ridge_Beta
MO_Pi_Ridge_1 <- Sim_Result$MO_Ridge_pi
MO_Pi_Ridge <- MO_Pi_Ridge_1[,1:3]

MO_Liu <- Sim_Result$MO_Liu_Beta
MO_Pi_Liu_1 <- Sim_Result$MO_Liu_pi
MO_Pi_Liu <- MO_Pi_Liu_1[,1:3]
