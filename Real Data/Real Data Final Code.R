...
Design_Tau <- function(x,y,beta,pi)
{
  n <- dim(x)[1]
  L <- length(pi)
  tau_Matrix <- matrix(data=0, nrow=n, ncol=L)
  for (j in 1:L) 
  {
    for (i in 1:n) 
    {
...
irwls_Liu_one_iteration_rid <- function(X, y, La ,dFix ,beta.k, beta.old)
{
  ...
  if(CN >= CN0) return(b_old2)     
  Xtz <- crossprod(X,W*z)
  CNxtx <- sqrt(abs(max(XtX)/min(XtX)))
...
Simulation <- function(epsilon,M,Beta,beta.0,beta.old,pi,pi.0,pi.old,maxit_EM,tol_irwls,printout=F)
{
  resArray_LS <-resArray_Ridge <- resArray_Liu <- array(NA,dim=c(2,2,Nsim))
  resArray_test <-array(NA,dim=c(n0test,3,Nsim))
  ...
  for (j in 1:Nsim)
  {
    ...
    Ind_train  <- sample(nrow(Design_Matrix),size = Nsample0,replace = T)
    Data_train <- Design_Matrix[Ind_train,]
    
   
    Ind_test   <- sample((1:182)[-Ind_train],size = n0test,replace = T)
    Data_test <- Design_Matrix[Ind_test,]
    resArray_test[,,j] <- Data_test
  ...
  return(list(MO_LS_Beta=resArray_LS,MO_LS_pi=MO_LS_pi,
              ...
              MO_Liu_Beta=resArray_Liu,MO_Liu_pi=MO_Liu_pi,M_O_test=resArray_test))
}
...
Wrapping <- function(M_hat_LS,M_hat_Ridge,M_hat_Liu,B_true,Pi_hat_LS,Pi_hat_Ridge,Pi_hat_Liu,Pi_true)
{
  ...
  Barray <- array(B_true,dim=c(2,2,Nsim))
...
.mse_classification <- function(Error,Sensitivity,Specificity)
{
  dif1 <- Error
  dif2 <- Sensitivity
  dif3 <- Specificity
  ...
  mean_err <- round(mean(dif1),2)
  mean_sen <- round(mean(dif2),2)
  mean_spe <- round(mean(dif3),2)
  tab <- c(mean_err,mean_sen,mean_spe)
  return(tab)
}


Testing <- function(MO_LS,MO_Ridge,MO_Liu,MO_Pi_LS,MO_Pi_Ridge,MO_Pi_Liu,M,MO_Test)
{
...
Sim_Result=Simulation(epsilon=epsilon_em0,M=M0,
...
Test_Result=Testing(MO_LS,MO_Ridge,MO_Liu,MO_Pi_LS,MO_Pi_Ridge,MO_Pi_Liu,M=M0,MO_Test)
Test_Result
